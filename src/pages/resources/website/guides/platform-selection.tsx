import React from 'react';
import { Link } from 'react-router-dom';
import { Server, ArrowRight, BookOpen } from 'lucide-react';
import ResourceLayout from '../../../../components/ResourceLayout';

const PlatformSelectionGuidePage: React.FC = () => {
  const topics = [
    {
      title: 'Business Requirements Analysis',
      items: [
        'Content Management Needs',
        'Technical Complexity',
        'Budget Constraints',
        'Growth Projections',
        'Team Capabilities',
      ],
    },
    {
      title: 'Platform Types',
      items: [
        'Content Management Systems',
        'Website Builders',
        'E-commerce Platforms',
        'Custom Development',
        'Headless Architecture',
      ],
    },
    {
      title: 'Evaluation Criteria',
      items: [
        'Scalability Options',
        'Security Features',
        'Performance Metrics',
        'Integration Capabilities',
        'Support Availability',
      ],
    },
    {
      title: 'Decision Framework',
      items: [
        'Feature Comparison',
        'Total Cost Analysis',
        'Technical Requirements',
        'User Experience Needs',
        'Maintenance Considerations',
      ],
    },
  ];

  return (
    <ResourceLayout>
      <div className="space-y-12">
        {/* Header */}
        <div className="border-b border-gray-200 pb-8">
          <div className="flex items-center space-x-4 mb-6">
            <Server className="w-10 h-10 text-primary-600" />
            <h1 className="text-3xl font-bold text-gray-900">Website Platform Selection Guide</h1>
          </div>
          <p className="text-xl text-gray-600 max-w-3xl">
            A comprehensive framework for selecting the right website platform that aligns with your business goals, technical requirements, and budget.
          </p>
        </div>

        {/* Main Content Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Topics */}
          <div className="lg:col-span-2 space-y-8">
            {topics.map((section, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">
                  {section.title}
                </h2>
                <ul className="space-y-3">
                  {section.items.map((item, itemIndex) => (
                    <li key={itemIndex} className="flex items-start">
                      <ArrowRight className="w-5 h-5 text-primary-600 mr-2 mt-0.5" />
                      <span className="text-gray-600">{item}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ))}
          </div>

          {/* Sidebar */}
          <div className="space-y-6">
            {/* CTA Card */}
            <div className="bg-primary-50 rounded-lg p-6">
              <BookOpen className="w-8 h-8 text-primary-600 mb-4" />
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                Need Platform Selection Help?
              </h3>
              <p className="text-gray-600 mb-4">
                Get expert assistance with selecting the right website platform for your specific business needs and technical requirements.
              </p>
              <Link
                to="/contact"
                className="block text-center px-4 py-2 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-primary-600 hover:bg-primary-700"
              >
                Contact Us
              </Link>
            </div>
          </div>
        </div>
      </div>
    </ResourceLayout>
  );
};

export default PlatformSelectionGuidePage;